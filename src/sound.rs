...
fn create_buffer_source(ctx: &AudioContext) -> Result<AudioBufferSourceNode> {
    ctx.create_buffer_source().map_err(|err| {
        anyhow!("Could not create buffer source: {:?}", err)
    })
}
...
fn connect_with_audio_node(
    ...
    buffer_source
        ...
        .map_err(|err| {
            anyhow!("Could not connect buffer source with destination: {:?}",err)
        })
}
...
pub async fn decode_audio_data(
    ...
    JsFuture::from(
        ...
        .map_err(|err| anyhow!("Could not convert audio data to a future: {:?}", err))?
        .dyn_into()
        .map_err(|err| anyhow!("Could not convert raw JSValue to ArrayBuffer: {:?}", err))
}
